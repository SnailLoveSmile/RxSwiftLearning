...
public extension Reactive where Base:UIViewController {
    ...
    //表示视图是否显示的可观察序列, 当VC显示状态变化时触发
    public var isVisiable: Observable<Bool> {
        let isSeen = self.base.rx.viewDidAppear.map{_ in true}
        let isMiss = self.base.rx.viewWillDisappear.map{_ in false}
        return Observable<Bool>.merge(isSeen, isMiss)
    }
